...
class Module
{
    ...
    public static function process(Request $request = null)
    {
        ...
        $modEnd = strpos($url, '/', 1);
        if ($modEnd === false) {
            $url = str_replace("/saml/module.php", null, $_SERVER["REQUEST_URI"]);
            $modEnd = strpos($url, '/', 1);
        }
        if ($modEnd === false) {
            $modEnd = strlen($url);
        ...
        if (!self::isModuleEnab$modEnd = strpos(\$url, '/', 1)led($module)) {
        ...
        if (!file_exists($path = false !== strpos($path, "?") ? explode("?", $path)[0] : $path)) {
            // file not found
